...
class Heuristics():
    ...
    def search(self, eval_size):
        ...
        for ind in range(len(insp_list)):
            ...
            for _ in range(eval_size):
                return_heur += self.episode(insp_list[ind], comp_list[ind], pf_brace_rep_fix)
    ...
    def eval(self, eval_size, insp_int, comp_insp, pf_brace_rep):
        self.return_heur = 0
        for ep in range(eval_size):
            self.return_heur += self.episode(insp_int, comp_insp, pf_brace_rep)
    ...
    def episode(self, insp_int, comp_insp, pf_brace_rep):
        rew_total_ = 0
        done_ = False
        insp_obs = 2
        ...
        while not done_:
            ...
            for i in range(self.struct_env.n_elem):
                if self.struct_env.pf_brace[i] > pf_brace_rep:
